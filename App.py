...
with main_tabs[0]:
    ...
    if "1.1" in sec1_sub:
        ...
        st.markdown("#### ⚡ 1.3 Cumulative Volume Delta (CVD) Line & Ultra-Red Volume Scanner")
    ...
    if "2.1" in sec2_sub:
        ...
        if not opt_df.empty:
            try:
                ...
                max_pain = round(curr_p / 20) * 20
                st.metric("CME Gold Options Max Pain Target", f"${max_pain:,.2f}", f"${max_pain - curr_p:+.2f}")
                ...
                fig_opt = go.Figure()
                fig_opt.add_trace(go.Bar(x=strikes, y=call_oi, name="Call OI", marker_color="#FF0000"))
                fig_opt.add_trace(go.Bar(x=strikes, y=put_oi, name="Put OI", marker_color="#228B22"))
                fig_opt.update_layout(barmode="group", height=400, template="plotly_dark")
